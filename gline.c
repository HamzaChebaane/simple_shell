...
/**
 * in_buffer - buffers chained commands
 ...
 * Return: bytes read
 */
ssize_t in_buffer(data_t *data, char **buf, size_t *len)
{
	ssize_t r = 0;
	size_t len_p = 0;
	...
	if (!*len) /* if nothing left in the buffer, fill it */
	{
		...
		*buf = NULL;
		signal(SIGINT, sHand);
#if USE_GLINE
		r = gline(buf, &len_p, stdin);
#else
		r = _gline(data, buf, &len_p);
#endif
		if (r > 0)
		{
			if ((*buf)[r - 1] == '\n')
...
ssize_t get_input(data_t *data)
{
	...
	*buf_p = buf; /* else not a chain, pass back buffer from _gline() */
	return (r); /* return length of buffer from _gline() */
}
 ...
 * Return: s
 */
int _gline(data_t *data, char **ptr, size_t *length)
{
